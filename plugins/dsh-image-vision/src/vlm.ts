 ...
 *
 * Ported from @dsh-external/dsh-vision (BSD-3-Clause, william-jin-cmu/dsh-vision).
 * @module dsh-image-vision/vlm
 */
...
export async function toImageUrl(source: string, maxImageBytes: number): Promise<string> {
  ...
  if (info.size > maxImageBytes) {
    throw new Error(`view_image: image is ${info.size} bytes, over the ${maxImageBytes}-byte limit (raise maxImageBytes in the plugin config)`)
  }
...
/**
 * Thinking-mode VLMs inline their reasoning as `<think>…</think>` in the
 * content. Strip it; a response that is ONLY an unterminated think block
 * (reasoning ate the token budget) becomes empty.
 */
...
/** One vision request's resolved parameters. */
